    ...
    return (
        <Link className="event-card" to={`/${event.url}`} >
            <div className="event-card-l">
                ...
                <h5 className="event-card-date">{event.location}</h5>
            </div>
            <div className="event-card-r">
                {event.organizer ? <h5 className="event-card-title">Presented by {event.organizer}</h5> : ""}
                {event.series ? <p className="italic">{event.series} Series</p> : ""}
